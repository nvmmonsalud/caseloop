...
export function AssignmentSetup({ initialSettings }: { initialSettings: PilotSettings }) {
  ...
  const [upload, setUpload] = useState<UploadedSource | null>(null);
  ...
  async function ingest(formData: FormData) {
    ...
    try {
      ...
      setUpload({ ...payload.source, filename: payload.filename, preview: payload.preview });
  ...
  function review(approve: boolean) {
    if (!upload) return;
    startReview(async () => {
      const result = await reviewCaseSourceAction(upload.id, approve);
      setUploadMessage(result.error || result.message);
      if (!result.error) setUpload(null);
    });
  }
    ...
    <section className="mt-12 border-y rule py-8" aria-labelledby="pilot-controls-title">
      ...
      <form action={saveAction} className="mt-8 paper rounded-2xl p-6 sm:p-8">
        ...
        <div className="mt-7 grid gap-5 sm:grid-cols-2">
          <label className="text-sm font-bold">Shared feedback title<input name="feedbackTitle" required maxLength={120} defaultValue={initialSettings.feedback?.title || "Class synthesis"} className="mt-2 w-full rounded-lg border rule bg-white p-3 font-normal" /></label>
          <label className="text-sm font-bold sm:col-span-2">Shared feedback<textarea name="feedbackBody" required maxLength={4000} rows={4} defaultValue={initialSettings.feedback?.body || "After class, summarize the evidence and assumptions the cohort should revisit."} className="mt-2 w-full rounded-lg border rule bg-white p-3 font-normal" /></label>
        </div>
        <label className="mt-5 flex items-start gap-3 rounded-xl bg-[#f1ede4] p-4 text-sm">
          <input type="checkbox" name="release" defaultChecked={Boolean(initialSettings.rubricReleasedAt)} className="mt-1" />
          <span><b>Release to students now</b><br /><span className="text-[#68727b]">Unchecked saves a faculty-only draft. Release is explicit and reversible.</span></span>
        </label>
      ...
      <div className="mt-8 rounded-2xl bg-[#10283f] p-6 text-white sm:p-8">
        ...
        {upload && <div className="mt-5 rounded-xl bg-white p-5 text-[#10283f]"><div className="text-xs font-bold text-[#8a6832]">{upload.sourceKey} · pending review · {upload.filename}</div><p className="mt-3 max-h-36 overflow-auto whitespace-pre-wrap text-sm leading-6">{upload.preview}</p><div className="mt-4 flex gap-3"><button disabled={reviewPending} type="button" onClick={() => review(true)} className="btn-primary">Approve for students</button><button disabled={reviewPending} type="button" onClick={() => review(false)} className="btn-secondary">Reject</button></div></div>}
      </div>
    </section>
  );
}
